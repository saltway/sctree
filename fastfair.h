...
static inline void persist(PMEMobjpool *pop,void *data, int len)
{
  // asm volatile("sfence" ::: "memory");

  // persist(pop, data, len);
  uintptr_t uptr;
...
public:
  ...
  inline bool remove_key(PMEMobjpool *pop, entry_key_t key)
  {
    ...
    for (i = 0; records[i].ptr != NULL; ++i)
    {
      ...
      if (shift)
      {
        ...
        if (do_flush)
        {
          persist(pop, (void *)records_ptr, CACHE_LINE_SIZE);
        }
      }
    }
      ...
      if (this == D_RO(bt->root))
      {
        if (hdr.level > 0)
        {
          if (num_entries_before == 1 && (hdr.sibling_ptr.oid.off == 0))
          {
            ...
            persist(bt->pop, &(bt->root), sizeof(TOID(page)));
    ...
    { // Redistribution
      ...
      { // left -> right
        if (hdr.leftmost_ptr == nullptr)
        {
          ...
          persist(bt->pop, &(D_RW(left_sibling)->records[m].ptr),
                          sizeof(char *));

          D_RW(left_sibling)->hdr.last_index = m - 1;
          persist(bt->pop, &(D_RW(left_sibling)->hdr.last_index),
                          sizeof(int16_t));
        ...
        else
        {
          ...
          persist(bt->pop, &(hdr.leftmost_ptr), sizeof(page *));
      ...
      { // from leftmost case
        hdr.is_deleted = 1;
        persist(bt->pop, &(hdr.is_deleted), sizeof(uint8_t));
        ...
        if (hdr.leftmost_ptr == nullptr)
        {
          ...
          persist(bt->pop, D_RW(new_sibling), sizeof(page));

          D_RW(left_sibling)->hdr.sibling_ptr = new_sibling;
          persist(bt->pop, &(D_RW(left_sibling)->hdr.sibling_ptr),
                          sizeof(page *));
                         ...
                         bool update_last_index = true)
  {
    ...
    { // this page is empty
      ...
      if (flush)
      {
        persist(pop, this, CACHE_LINE_SIZE);
      }
    }
    else
    {
      ...
      if (flush)
      {
        if ((uint64_t) & (records[*num_entries + 1]) % CACHE_LINE_SIZE == 0)
          persist(pop, &records[*num_entries + 1].ptr, sizeof(char *));
      }
      // FAST
      for (i = *num_entries - 1; i >= 0; i--)
      {
        ...
        else
        {
          ...
          if (flush)
            persist(pop, &records[i + 1], sizeof(entry));
          inserted = 1;
          break;
        }
      }
      if (inserted == 0)
      {
        ...
        if (flush)
          persist(pop, &records[0], sizeof(entry));
      }
    }
              ...
              bool with_lock, page *invalid_sibling = NULL)
  {
        ...
        ((page *)hdr.sibling_ptr.oid.off != invalid_sibling))
    {
      ...
      if (key >= D_RO(hdr.sibling_ptr)->hdr.lowest)
      {
        if (with_lock)
        {
          pthread_rwlock_unlock(hdr.rwlock);
    ...
    { // FAIR
      ...
      persist(bt->pop, sibling_ptr, sizeof(page));

      hdr.sibling_ptr = sibling;
      persist(bt->pop, &hdr, sizeof(hdr));
      ...
      if (IS_FORWARD(hdr.switch_counter))
        ...
        ++hdr.switch_counter;
      records[m].ptr = NULL;
      persist(bt->pop, &records[m], sizeof(entry));

      hdr.last_index = m - 1;
      persist(bt->pop, &hdr.last_index, sizeof(int16_t));
  ...
  page *update(btree *bt, char *left, entry_key_t key, char *right, bool flush,
               bool with_lock, page *invalid_sibling = NULL)
  {
    if (with_lock)
    {
        ...
        ((page *)hdr.sibling_ptr.oid.off != invalid_sibling))
    {
      ...
      if (key >= D_RO(hdr.sibling_ptr)->hdr.lowest)
      {
        ...
        return D_RW(hdr.sibling_ptr)
            ->update(bt, NULL, key, right, true, with_lock, invalid_sibling);
      }
    }
    if (records[0].key == key)
    {
      if ((records[0].ptr) != NULL)
      {
        records[0].ptr = right;
        persist(bt->pop, &records[0].ptr, 1);
        if (with_lock)
        {
          pthread_rwlock_unlock(hdr.rwlock);
        }
    ...
    for (int i = 1; records[i].ptr != NULL; ++i)
    {
      if (records[i].key == key)
      {
        if (records[i - 1].ptr != records[i].ptr)
        {
          records[i].ptr = right;
          persist(bt->pop, &records[i].ptr, 1);
          if (with_lock)
          {
            pthread_rwlock_unlock(hdr.rwlock);
          }
  ...
  char *linear_search(entry_key_t key, bool *is_sibling)
  {
    *is_sibling = false;
    int i = 1;
    uint8_t previous_switch_counter;
    ...
    {                                    // Search a leaf node
      ...
      if ((t = (char *)hdr.sibling_ptr.oid.off) &&
          key >= D_RW(hdr.sibling_ptr)->hdr.lowest)
      {
        *is_sibling = true;
        pthread_rwlock_unlock(hdr.rwlock);
        return t;
    ...
    Again:
      ...
      uint64_t temp_lowest;
      TOID(page)
      temp_page = hdr.sibling_ptr;
      if ((t = (char *)temp_page.oid.off) != NULL)
      {

        if (key >= (temp_lowest = D_RW(temp_page)->hdr.lowest))
        {
          *is_sibling = true;
          // ret = t;
          return t;
      ...
      if (ret)
      {
        TOID(page)
        node;
        TOID_ASSIGN(node, pmemobj_oid(this));
        node.oid.off = (uint64_t)ret;
        if (key < D_RW(node)->hdr.lowest)
        {
          goto Again;
        }
        return ret;
  ...
  void print()
  {
    ...
    printf("lowest: %ld\n", hdr.lowest);
    if (hdr.leftmost_ptr != NULL)
      printf("%ld ", hdr.leftmost_ptr);

    for (int i = 0; records[i].ptr != NULL; ++i)
      printf("%ld,%ld ", records[i].key, records[i].ptr);

    printf("%ld ", hdr.sibling_ptr.oid.off);
...
{
  root = new_root;
  persist(pop, &root, sizeof(TOID(page)));
  ++height;
}
...
char *btree::btree_search(entry_key_t key)
{
  ...
  while (D_RO(p)->hdr.leftmost_ptr != NULL)
  {
    ...
    p.oid.off = (uint64_t)D_RW(p)->linear_search(key, &is_sibling);
    // if(key < D_RW(p)->hdr.lowest ){
  ...
  while ((t = (uint64_t)D_RW(p)->linear_search(key, &is_sibling)) ==
  ...
  if (!t)
  {
    ...
    // // while (D_RW(p)->records[0].key != key)
    // // {
    // //   D_RW(p)->print();
    // //   p.oid.off = D_RW(p)->hdr.sibling_ptr.oid.off;
    // // }
    // FILE *fp = NULL;
    // fp = fopen("error.txt", "a");
    // fprintf(fp, "NOT FOUND %lu, t = %x\n", key, t);
    // D_RW(p)->print();
    // p.oid.off = D_RW(p)->hdr.sibling_ptr.oid.off;
    // D_RW(p)->print();
    // btree_search_range(key,1,NULL);
    // exit(0);
    return NULL;
  }
...
void btree::btree_update(entry_key_t key, char *right)
{
  TOID(page)
  p = root;
  bool is_sibling = false;
  ...
  D_RW(p)->update(this, NULL, key, right, true, true);
                                  ...
                                  uint32_t level)
{
  ...
  while (D_RO(p)->hdr.level > level)
  {
    p.oid.off = (uint64_t)D_RW(p)->linear_search(key, &is_sibling);
    // if(key < D_RW(p)->hdr.lowest ){
    //   puts("inner insert error");
    //   return btree_insert_internal(left, key, right, level);
    // }
  }
                               ...
                               unsigned long *buf, int number = 0)
{
  ...
  while (p.oid.off != 0)
  {
    if (D_RO(p)->hdr.leftmost_ptr != NULL)
    {
      ...
      p.oid.off = (uint64_t)D_RW(p)->linear_search(min, &is_sibling);
    }
    else
    {
      // Found a leaf
      D_RW(p)->linear_search_range(min, max, buf, number);
...
void btree::recover()
{
  ...
  if (root.oid.off)
  {
    do
    {
      ...
      while (sibling.oid.off)
      {
        D_RW(sibling)->hdr.rwlock = new pthread_rwlock_t;
        if (pthread_rwlock_init(D_RW(sibling)->hdr.rwlock, NULL))
        {
          perror("lock init fail");
          exit(1);
        }
        sibling.oid.off = D_RO(sibling)->hdr.sibling_ptr.oid.off;
      }
      leftmost.oid.off = (uint64_t)D_RO(leftmost)->hdr.leftmost_ptr;
    } while (leftmost.oid.off != 0);
  }
